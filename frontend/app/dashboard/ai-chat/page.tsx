...
// Utility function to format text with simple bold support
const formatText = (text: string): React.ReactNode => {
  // Split text by ** and __ patterns for bold formatting
...
export default function AstraChatPage() {
  ...
  useEffect(() => {
    ...
    // Listen for credential confirmation events
    const handleCredentialsConfirmed = (event: CustomEvent) => {
      console.log('Credentials confirmed:', event.detail)
      // You could add additional logic here if needed
    }
    
    window.addEventListener('credentialsConfirmed', handleCredentialsConfirmed as EventListener)
    
    return () => {
      window.removeEventListener('credentialsConfirmed', handleCredentialsConfirmed as EventListener)
    }
  }, [])
  ...
  // Debug credentials when they change
  useEffect(() => {
    console.log('Credentials changed:', { publicKey, privateKey, hasCredentials, isConfirmed })
  }, [publicKey, privateKey, hasCredentials, isConfirmed])
              ...
              <div className={cn(
                ...
                <div className={cn(
                  ...
                  {message.isLoading ? (
                    ...
                    )}>{formatText(message.content)}</div>
                  )}
